...
function generateSnake() {
    let posX = Math.round(Math.random() * (10 - 1) + 3);
...
let score = 0;
input.value = `Съедено жмыхов: ${score}`;
...
function move() {
    ...
    if (snakeBody[0].classList.contains('snakebody')) {
        setTimeout(function () {
            alert('Съедено жмыхов: ${score}');
        }, 200);
        clearInterval(interval);
        // snakeBody[0].classList.remove('head');
        snakeBody[0].style.background = 'url(img/ded-gadza.jpeg) center no-repeat';
